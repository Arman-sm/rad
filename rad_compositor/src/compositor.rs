...
use crate::{cmp_reg::{CompositorData, CompositorState}, composition::{CompositionSrc, TWrappedCompositionState, convert_sample_rates}, source::{BaseSource, Source, TFrameIdx, TFrameIdxSigned, TSample}};

const COMPUTE_AHEAD_SEC: f32 = 0.3;
const WAIT_TO_RECHECK_SEC: f32 = 0.05;
...
pub fn approximate_frame_linear(src: &mut Source, target_sample_rate: TFrameIdx, frame_i_in_target: TFrameIdx, offset: i64) -> Option<Vec<TSample>> {
	let conv = (frame_i_in_target * src.sample_rate()) as f64 / target_sample_rate as f64 - offset as f64;
	assert!(0.0 <= conv);
...
fn fetch_frame(cmp_src: &mut CompositionSrc, target_sample_rate: TFrameIdx, frame_idx: TFrameIdx) -> Option<Vec<TSample>> {
	let conv_frame_i =
		convert_sample_rates(
			target_sample_rate,
			frame_idx as TFrameIdxSigned,
			cmp_src.src.sample_rate()
		...
		.spawn(move || {
			...
			loop {
				...
				if COMPUTE_AHEAD_SEC < secs_sent - start.elapsed().as_f64() as f32 {
					sleep(Duration::from_secs_f32(WAIT_TO_RECHECK_SEC));
					continue;
				}

				{
					...
					if change_idx < cmp.config_change_idx {
						// TODO: Define what happens in such scenario (branch off and notify endpoints maybe?)
						// start = Instant::now();
						// secs_sent = 0.0;
				...
				secs_sent += frames_in_buf as f32 / sample_rate as f32;
